...
#TODO: This can be more efficient using median filter and differentiating the function
...
def levelshred(img, container_contour, label_contour=[], penalty_padding=[0.2, 0.2], kernel=5, savgol_size=-1, gt_window_size=15, threshold=1.5, bottle_debug_obj=None):
    ...
    # Apply gaussian blur
    # NOTE: THis kind of pre-processing should not be done in in this algorithm
    ...
    for i in range(0, len(viable_level_tally)):
        ...
        try:
            ...
            weighted_viable_level_tally[i] = 0
    ...
    lvshred_confidence = max_weighted_viable_level_tally + 100*(penalty_padding[0])#+0.5*penalty_padding[0]) # confidence is outputted as the portion of points to the width of the contour (+ padd]ing)
    # just incase, '-1' is our error marker
    lvshred_confidence = lvshred_confidence if lvshred_confidence < 100 else 99.9

    # We return 0 if waterlevel cannot be determined
    confidence_threshold=20
    if lvshred_confidence < confidence_threshold:
        fluid_level_y = 0
